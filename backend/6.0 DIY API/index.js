...
app.get("/filter", (req, res) => {
  ...
  const typeJokes = jokes.filter((joke) => joke.jokeType === type);
  res.json(typeJokes);
});
...
app.patch("/jokes/:id", (req, res) => {
  const id = parseInt(req.params.id);
  const currJoke = jokes.find((joke) => joke.id === id);
  const newJoke = {
    id: id,
    jokeText: req.body.text || currJoke.jokeText,
    jokeType: req.body.type || currJoke.jokeType,
  };
  const index = jokes.find((joke) => joke.id === id);
  jokes[index] = newJoke;
  res.json(newJoke);
});
